...
function resolveBin(): string | null {
  if (process.env.MC_KIMI_BIN) return process.env.MC_KIMI_BIN;
  const which = spawnSync("which", ["kimi"], { encoding: "utf8" });
  const path = which.status === 0 ? which.stdout.trim() : "";
  return path || null;
}

export const kimiCode: HarnessAdapter = {
  name: "kimi-code",
  ...
  buildCommand(ctx: LaunchContext) {
    ...
    const env: Record<string, string> = { KIMI_CODE_HOME: home };
    ...
    if (ctx.credential) env.KIMI_MODEL_API_KEY = ctx.credential.value;
    ...
    if (spec.model) env.KIMI_MODEL_NAME = spec.model;
