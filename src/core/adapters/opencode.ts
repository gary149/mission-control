...
function resolveBin(): string | null {
  if (process.env.MC_OPENCODE_BIN) return process.env.MC_OPENCODE_BIN;
...
export const opencode: HarnessAdapter = {
  ...
  buildCommand(ctx: LaunchContext) {
    ...
    if (spec.auth.mode === "gateway") {
      ...
      for (const sub of ["data", "config", "cache", "state"]) mkdirSync(join(home, sub), { recursive: true });
      ...
      if (ctx.credential) env.OPENROUTER_API_KEY = ctx.credential.value;
      ...
      if (spec.model) model = `openrouter/${spec.model}`;
    }
    ...
    if (ctx.resumeSessionId) argv.push("-s", ctx.resumeSessionId);
    argv.push(prompt);
    return { argv, env };
  },
  ...
  mapLine(line: string): MappedLine {
    ...
    const capture = () => {
      if (typeof obj.sessionID === "string" && obj.sessionID) update = { ...update, session_id: obj.sessionID };
    };
