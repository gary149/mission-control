...
function opencodeSubscriptionCheck(): string | null {
 ...
 */
function opencodeSubscriptionModelCheck(spec: RunSpec): void {
  if (!spec.model || !spec.model.includes("/")) {
    throw new PreflightError(
      `opencode requires --model with a provider prefix (e.g. anthropic/claude-sonnet-4-5): mc cannot vouch for its ambient default-model resolution`,
    );
  }
  const provider = spec.model.split("/")[0]!;
  try {
    ...
    if (!providers.includes(provider)) {
      throw new PreflightError(
        `opencode has no stored credential for provider "${provider}" (auth.json has: ${providers.join(", ") || "none"}); run \`opencode auth login\` to add it, or use --gateway openrouter`,
      );
    }
  } catch (error) {
    if (error instanceof PreflightError) throw error;
...
export function resolveAuth(spec: RunSpec, adapter: HarnessAdapter, config: McConfig): ResolvedAuth {
  ...
  if (spec.budget_usd != null && costBasis !== "metered_reported") {
    ...
    const anyMetered = adapter.capabilities.auth_modes.some(
      (m) => costBasisFor(adapter.name, m) === "metered_reported",
    );
    throw new PreflightError(
  ...
  if (mode === "subscription") {
    ...
    if (adapter.name === "opencode") opencodeSubscriptionModelCheck(spec);
    const credential =
  ...
  if (adapter.name === "opencode" && name !== "openrouter") {
    throw new PreflightError(
      `opencode gateway support is limited to "openrouter" in v0 (its builtin openrouter provider is used directly; a custom gateway would need a synthesized opencode.json custom-provider block, not yet implemented)`,
    );
  }
 ...
 */
export async function checkGatewayModelServed(spec: RunSpec, config: McConfig): Promise<void> {
  if (spec.auth.mode !== "gateway" || !spec.model) return;
  const name = spec.auth.gateway;
  const gatewayCfg = name ? config.gateways[name] : undefined;
  if (!gatewayCfg?.base_url_openai) return;
  const apiKey = process.env[gatewayCfg.env_var];
  if (!apiKey) return; // resolveAuth reports this properly; don't duplicate or race it here
  ...
  if (!Array.isArray(list) || list.length === 0) return; // empty/unexpected shape: nothing confident to refuse on
  const ids = new Set(list.map((m) => (m as { id?: unknown })?.id).filter((id) => typeof id === "string"));
  if (ids.size === 0 || ids.has(spec.model)) return;

  throw new PreflightError(
    `gateway "${name}" does not currently serve model "${spec.model}" ` +
