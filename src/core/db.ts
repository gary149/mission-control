...
import type { Assessment, EventKind, Run, RunEvent } from "./types.ts";
...
type RunRow = Omit<Run, "artifacts" | "notified"> & { artifacts: string; notified: number };

function rowToRun(row: RunRow): Run {
  return { ...row, artifacts: JSON.parse(row.artifacts), notified: row.notified === 1 };
}

function dollarKeys(obj: Record<string, unknown>): Record<string, unknown> {
...
export function updateRun(id: string, fields: Partial<Run>): void {
  ...
  if ("artifacts" in patch) patch.artifacts = JSON.stringify(patch.artifacts);
  if ("notified" in patch) patch.notified = patch.notified ? 1 : 0;
...
export function findRun(idOrPrefix: string): Run | null {
 ...
 */
export function claimNotify(id: string): boolean {
  const result = openDb()
...
type AssessmentRow = Omit<Assessment, "evidence" | "notified"> & { evidence: string; notified: number };
 ...
 */
export function latestAssessment(runId: string): Assessment | null {
