...
import { ensureHome, loadConfig, mcHome, type NotifyTarget } from "./core/config.ts";
import {
  assessmentsFor,
  eventsAfter,
  findRun,
...
async function initCheck(): Promise<void> {
  ...
  const targets: [string, NotifyTarget][] = [
...
async function runInit(args: string[]): Promise<void> {
  ...
  if (notifyExec || notifyWebhook) {
    text = upsertTomlSection(
      text,
      "notify",
      [notifyExec ? `exec = "${notifyExec}"` : null, notifyWebhook ? `webhook = "${notifyWebhook}"` : null].filter(
