...
function claudeSubscriptionCheck() {
    if (process.env.CLAUDE_CODE_OAUTH_TOKEN)
        return "CLAUDE_CODE_OAUTH_TOKEN (resident env)";
...
function codexSubscriptionCheck() {
    ...
    try {
        ...
        if (parsed?.tokens)
            return authFile; // ChatGPT login present (keys checked, values never read out)
    }
    catch {
        return null;
    }
 ...
 */
function piSubscriptionModelCheck(spec) {
    if (!spec.model || !spec.model.includes("/")) {
        throw new PreflightError(`pi requires --model with a provider prefix (e.g. openai-codex/gpt-5.5): its default model selection depends on ambient env, which mc strips`);
    }
    ...
    try {
        ...
        if (!providers.includes(provider)) {
            throw new PreflightError(`pi has no stored credential for provider "${provider}" (auth.json has: ${providers.join(", ") || "none"}); run \`pi\` interactively to add it, or use --gateway openrouter`);
        }
    }
    catch (error) {
...
export function resolveAuth(spec, adapter, config) {
    ...
    if (mode === "subscription") {
        ...
        if (!source) {
            throw new PreflightError(`no resident ${adapter.name} login found on this host; ${entry.hint}. Credentials are never bridged from another machine.`);
        }
        if (adapter.name === "pi")
    ...
    if (mode === "api_key") {
        ...
        if (!envVar)
            throw new PreflightError(`api_key mode is not defined for harness "${adapter.name}"`);
        const value = process.env[envVar];
        if (!value) {
            throw new PreflightError(`--api-key requires ${envVar} to be set on this host; it is not. ` +
    ...
    if (!gatewayCfg) {
        throw new PreflightError(`unknown gateway "${name}" (known: ${Object.keys(config.gateways).join(", ")}); add a [gateway.${name}] block to config.toml`);
    }
    if (!spec.model || !spec.model.includes("/")) {
        throw new PreflightError(`--gateway requires a provider-prefixed model id (e.g. moonshotai/kimi-k3) - got "${spec.model ?? ""}"`);
    }
    const value = process.env[gatewayCfg.env_var];
    if (!value) {
        throw new PreflightError(`gateway "${name}" requires ${gatewayCfg.env_var} to be set on this host; it is not. ` +
