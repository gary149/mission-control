...
export function createWorkdir(id, sourceCwd) {
    const dir = runDir(id);
    ...
    if (sourceCwd) {
        ...
        if (!isRepo) {
            ...
            throw new PreflightError(`--cwd ${abs} is not a git repository; v0's isolation model is git-worktree-based. ` +
 ...
 */
export function createWorkdirFromCheckpoint(id, parentWorkdir, at) {
    const dir = runDir(id);
    mkdirSync(dir, { recursive: true });
    const workdir = join(dir, "work");
    const ref = at ?? "HEAD";
    ...
    if (resolved.status !== 0) {
        throw new PreflightError(`checkpoint "${ref}" is not a commit in the parent run's workdir (${resolved.stderr?.trim() || "rev-parse failed"})`);
    }
