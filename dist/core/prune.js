...
const TERMINAL = ["succeeded", "failed", "killed", "lost"];
 ...
 */
function mainWorktreePath(workdir) {
 ...
 */
export function evaluateWorktree(run) {
 ...
 */
export function pruneWorktree(candidate) {
    ...
    const mainRepo = mainWorktreePath(workdir);
    if (!mainRepo)
        return { removed: false, error: "could not resolve the source repo to deregister the worktree" };
    const r = spawnSync("git", ["-C", mainRepo, "worktree", "remove", workdir], { encoding: "utf8" });
    if (r.status !== 0)
        return { removed: false, error: r.stderr?.trim() || "git worktree remove failed" };
    return { removed: true };
}
